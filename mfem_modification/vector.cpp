...
void Vector::Signcopy()
{
   const bool use_dev = UseDevice();
   const int N = size;
   auto y = ReadWrite(use_dev);
   mfem::forall_switch(use_dev, N, [=] MFEM_HOST_DEVICE (int i) { if(y[i] > 0.0){y[i] =1.0;} else{y[i]=-1.0;} });
}
...
void Multvv(const Vector &v1, const Vector &v2, Vector &v)
{
